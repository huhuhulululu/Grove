...
```mermaid
flowchart LR
    subgraph YOURS["🔒 あなたの実作業 · Grove からは読み取り専用"]
        direction TB
        A1["git commit コミット"]
        A2["あなたが回したテスト"]
        A3["あなたがマージした PR"]
        A4["CLAUDE.md · 仕様 · 同期された文書"]
    end
    YOURS -->|"アダプターは「成果」<br/>だけを捕捉"| E["📨 1 本の正規化<br/>イベントストリーム"]
    E --> ENGINE{{"🧪 純粋エンジン<br/>イベント ➜ 装飾的な状態<br/>ファイル無 · 時計無 · ネット無<br/>乱数 = 注入されたシード"}}
    ENGINE --> STATE["🎮 装飾的なゲーム状態<br/>XP · 🌰 シード · 🎴 カード<br/>⚔️ ギア · 🏆 実績 · 🗺️ 戦利品袋"]
    ENGINE -. "✋ 書き戻せない —— 純粋関数だから" .-> YOURS
...
矢印は**一方向のみ**。成果が*入り*、戦利品が*出る*。あなたのコード・コミット・git 履歴は、エンジンが越えられない壁の読み取り専用側にあります（ADR-0005）。
...
```mermaid
flowchart LR
    O["✅ 実際の成果を出荷<br/>テスト緑 · マージ · クリーンビルド · 仕様"]
    S["🌰 シード獲得<br/>使うタイミングはあなた次第"]
    SP["🎴 ガチャ · クラフト · フォイル<br/>⚔️ 強化 · 修復 · 保護<br/>🗺️ インカージョンに潜る"]
    U["✨ 装飾アップグレード<br/>コードには絶対触れない"]
    M["🌳 熟達 —— 温かい一度の到達<br/>トレッドミルではない"]
    O --> S --> SP --> U --> O
    U -. "やがて" .-> M
...
ゲーム全体は 2 本の柱 —— *疲労をやわらげる*、*良い習慣を促す* —— から育ち、ファイアウォールで囲われ、穏やかに保たれます：
...
flowchart TB
    ...
    G --> FW["🔒 倫理ファイアウォール"]
    G --> CAL["🔋 穏やかで、あなたのもの"]
    P1 --> a1["🎴 コレクション · 39 枚 · ガチャ · フォイル"]
    P1 --> a2["⚔️ ギア & ロードアウト · 強化 · シナジー"]
    P1 --> a3["🗺️ The Incursion · 運試しのダンジョン"]
    P1 --> a4["🏆 実績 · 熟達"]
    P2 --> b1["📜 クエストボード · CLAUDE.md · 仕様 · 文書 · ADR"]
    P2 --> b2["🎓 sq learn · オプトインの「なぜ」"]
    FW --> c1["純粋エンジン · 装飾のみ · コードや git に絶対触れない"]
    CAL --> d1["Vigor / Weekly エナジー · --zen · ローカルファースト · i18n ×4"]
...
| 🗺️ **The Incursion（ダンジョン）** | **ダンジョン**：運試しのローグライク。ビルドを携えてシード化された関門に潜り、さまざまな階層（**エリート**=高難度・高報酬 · **トレジャー**=安全な大当たり · **レスト**=回復）を進み、使い捨ての**シールド**を持ち込み、二段階の**ボス**に挑む —— ただし戦利品は**生きて脱出**してこそ自分のもの。深入りして死ねば袋ごと没収。リアルな賭けでも**100% 装飾のみ**：あなたのコード・コミット・git は決して触れられません（ADR-0005）。 |
...
| `sq incursion start [--kit shield]` · `dive` · `escape` · `history` | **ダンジョン**：シード化されたローグライクに潜り、階層アーキタイプと二段階ボスを抜ける。生きて脱出できたときだけ戦利品を獲得（装飾の賭け） |
